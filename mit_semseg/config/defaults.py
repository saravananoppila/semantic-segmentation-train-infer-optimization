...
_C.TRAIN.workers = 16
# use automatic mixed precision (fp16 autocast + grad scaling)
_C.TRAIN.amp = False
# gradient accumulation: optimizer steps once every N micro-batches (1 = off)
_C.TRAIN.accum_steps = 1
# gradient checkpointing on the encoder's HRNet stages (trade compute for activation memory)
_C.TRAIN.grad_checkpoint = False
# fused loss: decoder emits logits + F.cross_entropy (fuses log_softmax+NLLLoss into one kernel)
_C.TRAIN.fused_loss = False
# use the DALI GPU data pipeline (mixed decode + GPU resize/flip/normalize) instead of PIL+workers
_C.TRAIN.use_dali = False
# Exp 20: nvJPEG GPU decode — workers ship raw JPEG bytes, decode/resize/flip/normalize on GPU
_C.TRAIN.use_nvjpeg = False
# Clean fp32 baseline (OFAT Exp 0): disables every accepted optimization — channels_last, fused
# SGD, CUDA prefetcher + GPU-normalize, pin_memory, persistent_workers. Uses plain iter(loader)
# + CPU float normalization. For the baseline-vs-best comparison run.
_C.TRAIN.baseline = False
