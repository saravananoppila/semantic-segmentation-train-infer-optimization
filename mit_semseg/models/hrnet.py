...
class HRNetV2(nn.Module):
    def __init__(self, n_class, **kwargs):
        ...
        # Exp 18: when True, the multi-resolution stages are run under gradient
        # checkpointing (set externally from cfg.TRAIN.grad_checkpoint).
    ...
    def _run_stage(self, stage, x_list):
        # Exp 18: optionally checkpoint a multi-resolution stage. The stage takes/returns
        # a list of tensors, so we unpack the list into positional tensor args (which
        # checkpoint can track) and repack inside the closure. use_reentrant=False is
        # required for list I/O and preserves the BF16 autocast state on recompute.
        if self.grad_checkpoint and self.training:
            return cp.checkpoint(lambda *xs: stage(list(xs)), *x_list, use_reentrant=False)
        return stage(x_list)
